// PROTOTYPE B — flow shell: stage scaling, chrome, navigation and the one shared state.
import { useCallback, useEffect, useState } from "react";
import { Nebula } from "./nebula.jsx";
import { GLogo, Icon } from "./ui.jsx";
import { Step1, Step2, Step3, Step4 } from "./steps-1-4.jsx";
import { Step5, Step6, Step7, Step8 } from "./steps-5-8.jsx";
import { Finish } from "./finish.jsx";

const W = 1672,
  H = 941;
// Column split per panel, from grade.py SPLITS; the divider glides between them.
const SPLIT = [759, 801, 796, 727, 793, 816, 756, 777];
const LX = [46, 48, 48, 60, 47, 48, 46, 44];
const STEPS = [Step1, Step2, Step3, Step4, Step5, Step6, Step7, Step8];

export const INITIAL = {
  defaultAgent: "claude",
  views: { browser: true, docs: true, code: false, kanban: false, automate: false },
  caps: { browserUse: true, computerUse: "off", control: true, notify: true },
    ...
    <div className="page">
      <div className="stage" style={{ transform: `translate(-50%,-50%) scale(${scale})` }}>
        ...
        <div className="counter" style={{ left: rx - 120 }}>
          <b>{String(step).padStart(2, "0")}</b> <span>/ 08</span>
        </div>
        ...
        <div className="foot" style={{ left: lx, width: rx - lx }}>
          ...
          <div className="dots" role="tablist" aria-label="Steps">
            {STEPS.map((_, i) => (
              <button type="button" key={i} aria-label={`Step ${i + 1}`} aria-selected={i + 1 === step} onClick={() => go(i + 1)}>
                <i className={i + 1 === step ? "on" : i + 1 < step ? "past" : ""} />
              </button>
            ))}
          </div>
          <button type="button" className="next" disabled={step === 8} onClick={() => go(step + 1)}>
            Next
        ...
        {toastMsg && (
          <div className="toast" key={toastMsg.id} style={{ left: split + (W - split) / 2 }}>
      ...
      <div className="proto-pill">
        <span>Prototype B · Tone down</span>

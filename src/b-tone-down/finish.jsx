// PROTOTYPE B — what "Open Ghostex" lands on: a summary of every choice, and the opened workspace.
import { useEffect, useRef, useState } from "react";
import { AGENTS, AgentIcon, GLogo, Icon, at, RM } from "./ui.jsx";
import { Cta, Eyebrow, Headline, Sub } from "./steps-1-4.jsx";
import { base } from "./steps-5-8.jsx";

const VIEW_NAMES = { browser: "Browser", docs: "Docs", code: "Code", kanban: "Kanban", automate: "Automate" };
const CAP_NAMES = { browserUse: "Browser Use", control: "Control Ghostex", notify: "Notifications" };
...
export function Finish({ s, set, go, restart }) {
  ...
  const caps = Object.keys(CAP_NAMES).filter((k) => s.caps[k]).map((k) => CAP_NAMES[k]);
  if (s.caps.computerUse === "on") caps.push("Computer Use");
  if (s.caps.computerUse === "pending") caps.push("Computer Use (waiting for OS)");
  const connected = Object.keys(s.connected).filter((k) => s.connected[k]).map((k) => AGENTS[k].name);
  const rows = [
    ["Default agent", AGENTS[s.defaultAgent].name, 3],
    ["Workspace views", views.length ? views.map((v) => VIEW_NAMES[v]).join(", ") : "None, agents only", 4],
    ["Capabilities", caps.length ? caps.join(", ") : "Minimal setup", 5],
    ["Agent hooks", s.hooks ? `${connected.join(" + ")} connected` : "Skipped for now", 6],
    ["Mobile", s.mobileReady ? "Paired with this machine" : s.mobile.some(Boolean) ? "Partly set up" : "Not now", 7],
    ["Project", s.folder, 8],
    ["First session", agent === "terminal" ? "Terminal, no agent" : AGENTS[agent].name, 8],
    ["Session view", { chat: "Chat", terminal: "Terminal", remember: "Remember last" }[s.view], 8],
  ];
  if (s.installQueued) rows.push(["Install guide", "Opens after this screen", 3]);
      ...
      <div className="glass-n fwin" style={at(812, 92, 812, 704)}>
        ...
        <div className="fwin-body">
          <aside className="fwin-side">
            ...
            {s.hooks && <div className="fs-note">Hooks on: status and titles come from the agent.</div>}
          </aside>
          <div className="fwin-main">
              ...
              {lines.map(([who, t], i) =>
                asTerminal ? (
                  ...
                  <div key={i} className={"fb " + who}>
                    {who === "bot" && <span className="fb-who">{AGENTS[agent]?.name}</span>}
                    {t}
                  </div>
                ),
              )}
              {shown < SCRIPT[agent].length && <span className="typing">
                <i />
                <i />
                <i />
              </span>}
            ...
            <form className="fm-input" onSubmit={send}>
              <input id="finish-composer" value={draft} onChange={(e) => setDraft(e.target.value)} placeholder={agent === "terminal" ? "Type a command…" : `Message ${AGENTS[agent].name}`} />

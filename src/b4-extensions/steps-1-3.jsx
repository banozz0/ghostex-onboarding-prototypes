// PROTOTYPE B4 — panels 1–3: What Ghostex can do, Your agents, Workspace.
import { Fragment, useEffect, useRef, useState } from "react";
import { AGENTS, AgentIcon, Icon, InstallGuide, OtherStrip, Toggle, at } from "./ui.jsx";
import { ChatTerminalDemo, DesktopMobileDemo, TogetherDemo } from "./demos.jsx";
import { WorkspaceWindow } from "./workspace.jsx";
import { ComputerUsePreview, IntegrationPreview } from "./previews.jsx";
...
/**
 * An extension row: clicking it (or Enter/Space on it) only previews that extension. Its switch is the one
 * thing that flips it, and Toggle stops its own click, so the two never mix. Keys pressed on the switch itself
 * are left to the switch.
 */
export const previewRow = (preview) => ({
  role: "button",
  tabIndex: 0,
  onClick: preview,
  onKeyDown: (e) => {
...
// The right column's tabs: the entry scan, then one per extension on this panel.
const RTABS = [
  ["scan", "terminal", "Scan"],
  ["integration", "pulse", "Ghostex integration"],
  ["cu", "monitor", "Computer Use"],
];
// What switching each one on does. It reads beside the live preview, so no popup has to carry it.
const INTEGRATION_LEAD = "Ghostex adds a small helper to each agent's own settings, so it can show what every agent is doing.";
...
const CU_LEAD = "This hands over the whole machine: supported agents can see your screen and drive apps outside Ghostex.";
const CU_TERMS = [
...
/** An extension as it will look, with the sentences that say what switching it on does. */
function ExtensionPanel({ lead, terms, children }) {
  return (
    <div className="glass xpanel" style={at(885, 174, 700, 586)}>
      <p className="xp-lead">{lead}</p>
      <div className="xp-stage">{children}</div>
      <div className="xp-terms">
        {terms.map(([k, v]) => (
          <div key={k} className="xp-row">
            <em>{k}</em>
            <span>{v}</span>
          </div>
        ))}
...
export function Step2({ s, set, go, toast }) {
  ...
  const [rtab, setRtab] = useState("scan"); // what the right column shows: scan | integration | cu
  ...
  // A row click only previews its extension (Sven, 2026-09-10); the switch flips it and brings the same
  // preview up, so it is never flipped blind.
  const flipIntegration = () => {
  ...
  return (
    <>
      ...
      <div className={"glass vrow" + (live ? " on" : "")} style={at(48, 576, 706, 72)} {...previewRow(() => setRtab("integration"))}>
        ...
        <Toggle on={live} onClick={flipIntegration} label="Ghostex integration" />
      </div>
      <div className={"glass curow" + (cu ? " on" : "")} style={at(48, 660, 706, 72)} {...previewRow(() => setRtab("cu"))}>
        ...
        <Toggle size="md" on={cu} onClick={flipCu} label="Computer Use" />
      </div>
      ...
      {/* right: the scan console or an extension preview, picked by the tab strip; the tracker follows the scan */}
      <div className="xtabs" style={at(885, 122, 700, 40)} role="tablist" aria-label="Right side">
        {RTABS.map(([id, ic, label]) => (
          <button type="button" role="tab" key={id} aria-selected={rtab === id} className={"wtab" + (rtab === id ? " on" : "")} onClick={() => setRtab(id)}>
            <Icon n={ic} size={14} />
            {label}
          </button>
        ))}
      ...
      {rtab === "scan" && (
        <div className="glass term" style={at(885, 174, 700, 586)}>
      ...
      {rtab === "cu" && (
        <ExtensionPanel lead={CU_LEAD} terms={CU_TERMS}>
          <ComputerUsePreview on={cu} />
        </ExtensionPanel>
      )}
      <div className="track" style={at(900, 772, 670, 60)} aria-label="Progress">
...
export function Step3({ s, set, go, toast }) {
  ...
  // A view previewed from its row. The window shows it even while it is off, under a dashed tab, until a tab,
  // an agent or another view's switch takes the window somewhere else.
  ...
  const current = peek ?? (active === "agents" || v[active] ? active : ([...hist].reverse().find((id) => v[id]) ?? "agents"));
  const show = (id) => {
    setPeek(null);
    setActive(id);
  };
  const flip = (id) => {
    ...
    if (on) {
      ...
      setActive(id);
      // Switching the previewed view on keeps it previewed, so its dashed tab turns real in place.
      setPeek((p) => (p === id ? p : null));
    }
  };
  // The browser skill only shows in the Browser tab, so flipping it goes there.
  const flipSkill = () => {
    if (!v.browser) return;
    set({ browserUse: !s.browserUse });
    show("browser");
  };
  const recommend = () => {
    ...
    show(added[added.length - 1]);
  };

  const row = (r, y, h) => (
    <div key={r.id} className={"glass vrow" + (v[r.id] ? " on" : "")} style={at(60, y, 632, h)} {...previewRow(() => setPeek(r.id))}>
  ...
  return (
    <>
      ...
      <div className={"glass vgroup" + (v.browser ? " on" : "")} style={at(60, 340, 632, 112)}>
        <div className="vrow inner" {...previewRow(() => setPeek("browser"))}>
        ...
        <div className={"vsub" + (v.browser ? "" : " off")} {...previewRow(() => setPeek("browser"))}>
          ...
          <Toggle size="sm" on={drive} disabled={!v.browser} onClick={flipSkill} label="Give agents the browser skill" />
        </div>
      </div>
      ...
      {/* right: the workspace, one titlebar tab per view that is on, and a dashed one for a previewed view that is off */}
      <WorkspaceWindow
        views={v}
        drive={drive}
        current={current}
        onTab={(id) => id !== current && show(id)}
        sel={sel}
        onAgent={(id) => {
          setSel(id);
          show("agents");

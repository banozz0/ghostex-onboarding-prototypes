// PROTOTYPE B4 — panels 4–5: Mobile, Get started.
import { useEffect, useRef, useState } from "react";
import { AgentIcon, GLogo, Icon, QR, Toggle, at, useOutside, RM } from "./ui.jsx";
import { Cta, Eyebrow, Headline, Sub, previewRow } from "./steps-1-3.jsx";
...
function PhoneApp({ toast, notify }) {
  const [view, setView] = useState("home");
  const [agent, setAgent] = useState("claude");
  // "Ping me" is previewed here and on ComputerCard: switching it drops the alert on both, or takes both away.
  const [banner, setBanner] = useState(notify);
  useEffect(() => setBanner(notify), [notify]);
      ...
      {view === "session" ? (
        ...
        <>
          <div className="ph-head">
            ...
            <button type="button" className={"ph-bell" + (notify ? "" : " off")} onClick={() => setBanner((b) => notify && !b)} aria-label="Notifications">
              <Icon n="bell" size={20} />
              <i />
            </button>
          </div>
...
/** This computer's half of "Ping me": the phone banner's alert, as a desktop notification over the window's top-right. */
function DesktopAlert() {
  return (
    <div className="pc-notif" role="status" style={at(926, 178, 300, undefined)}>
      <AgentIcon id="codex" size={20} />
...
/**
 * The computer's side of the demo: Settings → Remote, with the one password prompt. Once the demo has finished
 * and "Ping me" is off, its titlebar carries the quiet line the phone's header does.
 */
function ComputerCard({ t, notify }) {
  const ec = t >= T.toggle;
  ...
  return (
    <div className="glass pc" style={at(806, 246, 420, 448)}>
      <div className="pc-bar">
        ...
        <b>Remote</b>
        {ended && !notify && <span className="pc-quiet">Notifications off</span>}
...
export function Step4({ s, set, go, toast }) {
  ...
  const ended = t >= T.end;
  // The alerts only show once pairing has finished, so previewing or flipping "Ping me" runs the demo out to it
  // first. Only the switch changes the setting (Sven, 2026-09-10).
  const previewNotify = () => {
    if (!ended) jump(T.end);
  };
  const flipNotify = () => {
  ...
  return (
    <>
      ...
      <div className={"glass vrow" + (s.notify ? " on" : "")} style={at(46, 608, 660, 72)} {...previewRow(previewNotify)}>
        <Icon n="bell" size={22} className="vicon" />
        <div>
          <div className="nm lg">Ping me when an agent needs me</div>
          <div className="ss">
            {s.notify ? "This computer and your phone alert you when an agent needs an answer." : "No alerts on this computer or your phone: you find out when you look."}
          </div>
        </div>
        <Toggle on={s.notify} onClick={flipNotify} label="Ping me when an agent needs me" />
      </div>
      ...
      <ComputerCard t={t} notify={s.notify} />
      {ended && s.notify && <DesktopAlert />}
      ...
      <div className="devices" onPointerMove={move} onPointerLeave={() => setTilt({ x: 0, y: 0 })}>
        <div className="phone" style={{ transform: `perspective(1400px) rotateY(${-6 + tilt.y}deg) rotateX(${2 + tilt.x}deg) rotateZ(1deg)` }}>
          ...
          {ended ? <PhoneApp toast={toast} notify={s.notify} /> : <PhoneDemo t={t} />}
        </div>
      </div>
      {ended && (

// PROTOTYPE B2 — panel 3's right side: the workspace window, with one titlebar tab per view that is on.
import { useEffect, useState } from "react";
import { AgentIcon, GLogo, Icon, Menu, RM, at, useNow } from "./ui.jsx";

// The real app's titlebar order after Agents.
const TAB_ORDER = ["code", "browser", "kanban", "automate", "docs"];
const META = {
  agents: ["users", "Agents"],
  code: ["code", "Code"],
...
function BrowserView({ drive, toast }) {
  ...
  return (
    <div className="wv">
      ...
      {drive && <div className="wv-foot">Your agent is using the browser skill.</div>}
    </div>
  );
}
...
export function WorkspaceWindow({ views, drive, current, onTab, sel, onAgent, toast }) {
  const tabs = ["agents", ...TAB_ORDER.filter((id) => views[id])];
  const view = {
    agents: <SessionView agent={sel} />,
    browser: <BrowserView drive={drive} toast={toast} />,
    docs: <DocsView />,
    code: <CodeView />,
    kanban: <KanbanView />,
    automate: <AutomateView toast={toast} />,
  }[current];
  return (
    <>
      ...
      <div className="wsbar" style={at(758, 116, 875, 54)}>
        ...
        <div className="wtabs" role="tablist" aria-label="Workspace views">
          {tabs.map((id) => (
            <button type="button" role="tab" key={id} aria-selected={current === id} className={"wtab" + (current === id ? " on" : "")} onClick={() => onTab(id)}>

// PROTOTYPE — shared primitives for the focused onboarding (version B2).
...
/**
 * "How it works": a small link that opens an explainer card at fixed stage coordinates.
 * The stage is transformed, so position:fixed resolves against it and the card escapes any row that clips.
 */
export function HowItWorks({ title, items, pop }) {
  const [open, setOpen] = useState(false);
  const ref = useRef(null);
  useOutside(ref, () => setOpen(false), open);
  return (
    <span className="hiw" ref={ref} onClick={(e) => e.stopPropagation()}>
      <button type="button" className="hiw-btn" aria-expanded={open} onClick={() => setOpen((o) => !o)}>
        <Icon n="info" size={13} />
        How it works
      </button>
      {open && (
        <span className="hiw-pop" role="tooltip" aria-label={title} style={{ left: pop[0], top: pop[1], width: pop[2] }}>
          <b className="hiw-t">{title}</b>
          {items.map(([k, v]) => (
            <span key={k} className="hiw-row">
              <em>{k}</em>
              {v}
            </span>
          ))}
